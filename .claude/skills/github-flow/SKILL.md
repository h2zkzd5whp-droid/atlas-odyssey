...
9. **리뷰** — 마지막 푸시 후에 `/code-review <PR번호>`를 돌린다. 결과가 비어 와도 "문제 없음"으로 보지 않는다 (low 리뷰는 문서 렌더링 문제를 놓친 적이 있다). 실제 관문은 직접 리뷰다: `gh pr diff <PR번호>` 전체를 읽고, 문서는 렌더링(링크 앵커, 코드 스팬)까지 확인한다. 발견 사항이 있으면 3단계로 돌아가 고친다. 끝나면 PR 체크리스트를 갱신한다
